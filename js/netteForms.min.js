...
Nette.getValue=function(a){var b,c;if(a){if(a.nodeName){if(a.form.elements[a.name].nodeName){if("select"===a.nodeName.toLowerCase()){b=a.selectedIndex;var d=a.options,e=[];if("select-one"===a.type)return 0>b?null:d[b].value;b=0;for(c=d.length;b<c;b++)d[b].selected&&e.push(d[b].value);return e}return"checkbox"===a.type?a.checked:"radio"===a.type?a.checked&&a.value:"file"===a.type?a.files||a.value:a.value.replace("\r","").replace(/^\s+|\s+$/g,"")}return Nette.getValue(a.form.elements[a.name])}d=a[0]&&
!!a[0].name.match(/\[\]$/);e=[];b=0;for(c=a.length;b<c;b++)if(!(a[b].type in{checkbox:1,radio:1})||a[b].checked)if(d)e.push(a[b].value);else return a[b].value;return d?e:null}return null};Nette.getEffectiveValue=function(a){var b=Nette.getValue(a);a.getAttribute&&b===a.getAttribute("data-nette-empty-value")&&(b="");return b};
